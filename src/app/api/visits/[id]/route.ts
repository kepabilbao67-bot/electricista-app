...
export async function PUT(
  ...
  try {
    ...
    await db.execute({
      sql: `UPDATE visits SET title = ?, description = ?, date = ?, time = ?, duration = ?, status = ?, address = ?, notes = ?, client_id = ?, title_color = ?, description_color = ?, address_color = ?, updated_at = datetime('now')
       WHERE id = ?`,
      args: [
        body.title,
