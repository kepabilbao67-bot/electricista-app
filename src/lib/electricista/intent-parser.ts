/**
 * ELECTRICISTA360 — Intent Parser
 * Parser determinista y local. No llama APIs externas y siempre devuelve drafts.
 */
...
function extractBudgetLines(text: string): BudgetLineField[] {
  ...
  while ((m = totalPattern.exec(text)) !== null) {
    ...
    if (capturedRanges.some(([start, end]) => matchStart >= start && matchStart < end)) continue;
    const total = parseFloat(m[1].replace(",", "."));
    const description = m[2].trim();
    const isDuplicate = lines.some(
      (l) => Math.abs(l.quantity * l.unitPrice - total) < 0.01 || Math.abs(l.unitPrice - total) < 0.01
    );
...
export function parseIntent(input: string): ParsedIntent {
  ...
  if (!text) {
    return { type: "unknown", security: "READ", confidence: 0, fields: {}, rawInput: input, missingFields: [], isDraft: true };
  }
  ...
  if (detectedType === "create_budget" || detectedType === "create_invoice") {
    ...
    const budgetFields: CreateBudgetFields = { clientName, lines, taxRate };
